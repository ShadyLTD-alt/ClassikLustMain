...
export default function CharacterSelector({ isOpen, onClose }: CharacterSelectorProps) {
  ...
  // Fetch player data to get current character and username
  ...
  const currentCharacter = characters.find((c: Character) => c.id === player?.selectedCharacterId);
  ...
  return (
    <>
        ...
        <DialogContent className="max-w-4xl max-h-[90vh] bg-gray-900/95 backdrop-blur-lg text-white border-purple-500/50 overflow-hidden">
          <DialogHeader>
            ...
            <div className="flex items-center gap-3 p-3 bg-black/40 rounded-lg border border-gray-700/50">
              <div className="relative">
                {currentCharacter ? (
                  <img
                    ...
                    onError={(e) => { (e.target as HTMLImageElement).src = '/uploads/placeholder-avatar.jpg'; }}
                  />
                ) : (
              ...
              <div className="flex-1">
                <div className="text-sm text-gray-400">@{player?.username || 'Unknown'}</div>
                <div className="text-lg font-semibold">{currentCharacter ? currentCharacter.name : 'No character selected'}</div>
                <div className="text-sm text-purple-400">Level {player?.level || 1} • {Math.floor(player?.points || 0)} points</div>
              </div>
              {currentCharacter && (
                <Button variant="outline" size="sm" onClick={() => { setSelectedCharacterId(currentCharacter.id); setShowImageGallery(true); }} className="border-purple-400/50 text-purple-400 hover:bg-purple-600/20">
          ...
          <div className="space-y-4">
            ...
            <Tabs value={filter} onValueChange={(v: any) => setFilter(v)} className="w-full">
              ...
              {/* Dummy contents to satisfy Tabs API; actual filtering is handled via state */}
              ...
              {charactersLoading ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4">
                  {filteredCharacters.map((char: Character) => {
                    ...
                    return (
                      <Card key={char.id} className={`cursor-pointer transition-all hover:scale-105 relative ${isSelected ? 'ring-2 ring-purple-400 bg-purple-900/30 border-purple-400' : isUnlocked ? 'bg-gray-800/60 hover:bg-gray-700/80 border-gray-600' : 'bg-gray-900/60 border-gray-700 opacity-75'}`} onClick={() => handleSelectCharacter(char.id)}>
                        <CardContent className="p-4 text-center">
                          <div className="relative mb-3">
                            <img src={char.defaultImage || char.avatarImage || '/uploads/placeholder-avatar.jpg'} alt={char.name} className="w-20 h-20 mx-auto rounded-full object-cover" onError={(e) => { (e.target as HTMLImageElement).src = '/uploads/placeholder-avatar.jpg'; }} />
                            {!isUnlocked && (<div className="absolute inset-0 bg-black/70 rounded-full flex items-center justify-center"><Lock className="w-8 h-8 text-red-400" /></div>)}
                            {isSelected && (<div className="absolute -top-2 -right-2 bg-purple-600 rounded-full p-1"><User className="w-4 h-4 text-white" /></div>)}
                            <div className="absolute -bottom-1 -right-1">{getCharacterIcon(char)}</div>
                          </div>
                          <h3 className="font-semibold text-sm mb-1">{char.name}</h3>
                          <div className="space-y-1">
                            <div className="text-xs text-gray-400">Unlock Level {char.unlockLevel || 1}</div>
                            {isUnlocked ? (
                              <Badge variant="secondary" className="bg-green-600/20 text-green-400 text-xs"><Unlock className="w-3 h-3 mr-1" />Unlocked</Badge>
                            ) : (
                              <Badge variant="secondary" className="bg-red-600/20 text-red-400 text-xs"><Lock className="w-3 h-3 mr-1" />Locked</Badge>
                            )}
                          </div>
                          {isUnlocked && (
                            <Button variant="outline" size="sm" className="mt-2 w-full text-xs border-purple-400/50 text-purple-400 hover:bg-purple-600/20" onClick={(e) => { e.stopPropagation(); setSelectedCharacterId(char.id); setShowImageGallery(true); }}>
      ...
      <CharacterGallery isOpen={showImageGallery} onClose={() => setShowImageGallery(false)} userId={player?.id || ''} currentCharacterId={selectedCharacterId} onCharacterSelected={(characterId: string) => { handleSelectCharacter(characterId); setShowImageGallery(false); }} />
    </>
  );
}
