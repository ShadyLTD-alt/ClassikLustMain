...
export default function ImageUploaderCore() {
  ...
  const addGlobalPose = () => {
    ...
    setNewPoseName('');
    console.log('鉁� [POSES] Added global pose:', trimmed);
  };
  ...
  const removeGlobalPose = (pose: string) => {
    ...
    setGlobalPoses(updated);
    savePersistentPoses(updated);
    console.log('鉂� [POSES] Removed global pose:', pose);
  };
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-purple-900/20 border border-purple-500/30 rounded-lg p-4">
        <h4 className="text-sm font-semibold text-purple-300 mb-3">馃彿锔� Global Poses Library</h4>
      ...
      >
        ...
        <p className="text-sm text-gray-400 mb-4">Auto-sorted by character 鈫� type (e.g. /aria/avatar/)</p>
        <label
          htmlFor="file-upload"
        ...
        <div className="space-y-4 bg-gray-900/50 p-4 rounded-lg border border-purple-500/30">
          <div className="flex justify-between items-center">
            <h4 className="text-md font-semibold text-purple-300">
              馃摛 Ready to Upload ({pendingUploads.length} file{pendingUploads.length > 1 ? 's' : ''})
            </h4>
            <button
      ...
      <div>
        <h4 className="text-md font-semibold text-white mb-3">
          馃柤锔� Recent Uploads {images.length > 0 && `(${images.length})`}
        </h4>
        ...
        {images.length > 8 && (
          <p className="text-sm text-gray-400 text-center mt-4">
            Showing 8 of {images.length} images. View all in Gallery tab 鈫�
          </p>
        )}
      </div>
    </div>
