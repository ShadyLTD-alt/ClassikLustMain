        ...
        <div className="space-y-6">
          ...
          <div className="p-4 bg-gray-800/30 rounded-lg border border-gray-700/50">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-gray-400 text-sm">
                {Math.round(state.experience)} / {experienceToNextLevel} XP
              </span>
            </div>
            <Progress value={experienceProgress} className="h-3 mb-2" />
            <div className="text-xs text-gray-400">
              {Math.round(experienceToNextLevel - state.experience)} XP needed for Level {state.level + 1}
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="p-3 bg-gray-800/30 rounded-lg border border-gray-700/50">
              <div className="text-lg font-bold text-white">{Math.round(state.totalTapsAllTime || 0).toLocaleString()}</div>
              <div className="text-gray-400 text-sm">Total Taps</div>
            </div>
            <div className="p-3 bg-gray-800/30 rounded-lg border border-gray-700/50">
              <div className="text-lg font-bold text-white">{Object.keys(state.upgrades || {}).length}</div>
              <div className="text-gray-400 text-sm">Upgrades Owned</div>
            </div>
            <div className="p-3 bg-gray-800/30 rounded-lg border border-gray-700/50">
              <div className="text-lg font-bold text-white">{(state.unlockedCharacters || []).length}</div>
              <div className="text-gray-400 text-sm">Characters Unlocked</div>
            </div>
            <div className="p-3 bg-gray-800/30 rounded-lg border border-gray-700/50">
              <div className="text-lg font-bold text-white">{Math.round(state.consecutiveDays || 0)}</div>
          ...
          <div className="p-4 bg-gray-800/30 rounded-lg border border-gray-700/50">
            ...
            <div className="grid grid-cols-4 gap-4 text-sm">
              ...
              <div className="text-center">
                <div className="text-white font-bold">{Math.round(state.lpEarnedToday || 0)}</div>
