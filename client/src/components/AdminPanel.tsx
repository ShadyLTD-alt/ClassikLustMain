...
export default function AdminPanel() {
  ...
  const saveUpgradeMutation = useMutation({
    mutationFn: async (upgrade: UpgradeConfig) => {
      ...
      const response = await fetch(url, { method, headers, body: safeStringify(cleanedData) });
      if (!response.ok) {
  ...
  const updatePlayerMutation = useMutation({
    mutationFn: async ({ playerId, updates }: { playerId: string, updates: any }) => {
      ...
      const response = await fetch(`/api/admin/players/${playerId}`, { method: 'PATCH', headers, body: safeStringify(cleanedUpdates) });
      if (!response.ok) {
        const errorText = await response.text();
  ...
  const handleCreateLevel = () => {
    ...
    const newLevel: LevelConfig = { level: nextLevel, cost: 100 * nextLevel, requirements: [], unlocks: [] };
          ...
          <div className="flex-1 overflow-hidden">
            <Tabs defaultValue="upgrades" className="h-full flex flex-col">
              ...
              <TabsContent value="upgrades" className="flex-1 overflow-hidden">
                ...
                <ScrollContainer height="h-[500px]">
              ...
              <TabsContent value="levels" className="flex-1 overflow-hidden">
                ...
                <ScrollContainer height="h-[400px]">
                  <div className="space-y-3 pr-4">
              ...
              <TabsContent value="players" className="flex-1 overflow-hidden">
                <div className="space-y-4">
                  ...
                  {playersLoading ? (
                    ...
                    <ScrollContainer height="h-[450px]">
                      <div className="space-y-3 pr-4">
