-- Migration: Add LustGems and Boost System Fields (FIXED NAMING)
-- Date: 2025-11-02
...
CREATE INDEX IF NOT EXISTS idxPlayersLustgems ON players(lustGems);
CREATE INDEX IF NOT EXISTS idxPlayersBoostExpires ON players(boostExpiresAt) WHERE boostActive = true;
CREATE INDEX IF NOT EXISTS idxPlayersTapsToday ON players(totalTapsToday);
...
CREATE TABLE IF NOT EXISTS boostPurchases (
...
-- Indexes for boostPurchases
CREATE INDEX IF NOT EXISTS idxBoostPurchasesPlayer ON boostPurchases(playerId);
CREATE INDEX IF NOT EXISTS idxBoostPurchasesExpires ON boostPurchases(expiresAt) WHERE expiresAt IS NOT NULL;
CREATE INDEX IF NOT EXISTS idxBoostPurchasesPurchased ON boostPurchases(purchasedAt);

-- View for active boosts
CREATE OR REPLACE VIEW activeBoosts AS
SELECT 
    bp.*,
    p.username,
    CASE 
...
CREATE OR REPLACE FUNCTION cleanupExpiredBoosts()
RETURNS INTEGER AS $$
DECLARE
...
COMMENT ON FUNCTION cleanupExpiredBoosts() IS 'Cleans up expired boost states from players table';

-- Grant permissions
GRANT SELECT, INSERT, UPDATE ON boostPurchases TO authenticated;
GRANT SELECT ON activeBoosts TO authenticated;
GRANT EXECUTE ON FUNCTION cleanupExpiredBoosts() TO service_role;