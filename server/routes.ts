...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  // 🚀 NEW: Level-up endpoint with DB sync to playerLevelUps
  app.post("/api/player/level-up", requireAuth, async (req, res) => {
    try {
      ...
      try {
        await withTimeout(
          storage.recordPlayerLevelUp(req.player!.id, nextLevel, 'progression'),
          5000,
  ...
  // [REST OF THE EXISTING ROUTES FROM THE CURRENT FILE...]
  ...
  console.log('🔧 CRITICAL: updatePlayerState runtime error FIXED');
  return httpServer;
}